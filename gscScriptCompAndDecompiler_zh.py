...
class GscFile:
    ...
    def __init__(self, FileName, Mode):
        ...
        if (Mode == 0):
            ...
            self.File = open(self.FileName + ".ini", mode="r", encoding="gbk")
    ...
    def ReadStringDef(self):
        ...
        for i in range(0, len(self.FileStringOffsets)):
            ...
            self.FileStrings.append(self.File.read(Dohod-1).decode("gbk"))
            self.File.read(1)
    ...
    def RemakeOffsetsFromStrings(self):
        ...
        for i in range(0, len(self.FileStrings)):
            ...
            Dohod += 1 + len(self.FileStrings[i].encode("gbk"))
    ...
    def RewriteStringDef(self):
        ...
        for i in self.FileStrings:
            self.FileStruct[3] += i.encode("gbk") + b'\x00'
    ...
    def WriteFile(self):
        self.File = open(self.FileName + ".ini", mode="w", encoding="gbk")
...
class GUI():
    ...
    def InitLeftSide(self):
        ...
        self.Decompile = Button(self.SpacerTop, command=self.DecompileToIni, bg='white', activebackground='gray', font = 'calibri 12', text='Decompile .gsc to .ini (.gsc -> .ini)')
        self.Decompile.pack(side='top', fill="x", expand='yes')
        self.Compile = Button(self.SpacerTop, command=self.CompileFromIni, bg='white', activebackground='gray', font = 'calibri 12', text='Compile .ini to .gsc (.ini -> .gsc)')
    ...
    def SetLangChs(self):
        self.Language = "CHS"
        self.root.title("Gsc 3.0")
        self.Definer['text'] = "选定文件"
        self.Clearer['text'] = " 清除文本"
        self.Undefiner['text'] = "解除选定"
        self.SpacerTop['text'] = '命令：'
        self.Rebuild['text'] = 'gsc文件重新构建 (.gsc -> .gsc)'
        self.Decompile['text'] = 'gsc文件转ini (.gsc -> .ini)'
        self.Compile['text'] = 'ini文件转gsc (.ini -> .gsc)'
        self.SpacerBottom['text'] = '说明：'
        self.CommonHelper['text'] = '程序介绍'
        self.CommandHelper['text'] = '命令含义'
        self.SyntaxHelper['text'] = '语法解释'
        self.Outer['state'] = NORMAL
        self.Outer.delete(1.0, END)
        self.Outer.insert(1.0, "大概翻译一下")
    ...
    def SetLangEng(self):
        ...
        self.Decompile['text'] = 'Decompile .gsc to .ini (.gsc -> .ini)'
        self.Compile['text'] = 'Compile .ini to .gsc (.ini -> .gsc)'
    ...
    def Clear(self):
        if (self.InputName['state'] == NORMAL):
            ...
            if (self.Language == 'CHS'):
                self.Outer.insert(1.0, "已成功清除名称输入字段")
            else:
            ...
            self.Outer.delete(1.0, END)
            if (self.Language == 'CHS'):
                self.Outer.insert(1.0, "清除失败，请先解除选定")
            else:
    ...
    def Define(self):
        ...
        try:
            if ((self.FileName[-4:] == ".gsc") or (self.FileName[-4:] == '.ini')):
    ...
    def Undefine(self):
        ...
        self.Outer.delete(1.0, END)
        if (self.Language == 'CHS'):
            self.Outer.insert(1.0, "成功解除选定")
        else:
            self.Outer.insert(1.0, "The name entry was succesfully undefined.")    
        self.Outer['state'] = DISABLED      
    def CommonHelp(self):
        if (self.Language == "CHS"):
            mb.showwarning("程序介绍", """这个程序能用来处理骗子社codeX-Rscript引擎的gsc脚本文件。虽说使用简单，但这个程序并不万能，译者用来处理腐姬归省时就失败了。好了，想进一步了解就去看英文帮助吧，或者自己摸索几下也就会用了。另外，本程序相比原程序有两处改动：\n1、原程序只支持日语编码，这里我就多创了个gbk的脚本，要编成中文就要用那个脚本啦。\n2、原程序是gsc和txt互转，我这里改成ini来更好支持vscode的语法高亮""")
        else:
    ...
    def CommandHelp(self):
        if (self.Language == "CHS"):
            mb.showwarning("命令含义", """这位置怎么放得下？去看readme吧""")
        else:
    ...
    def RebuildGscFromGsc(self):
        try:
            ...
            self.Outer['state'] = DISABLED
    def DecompileToIni(self):
        NewScript = GscFile(self.FileName, 0)
        NewScript.ReinitAll()
        NewScript.FileName = self.FileName
        NewScript.DecompileGscToIni()
        try:
            #NewScript = GscFile(self.FileName, 0)
            #NewScript.ReinitAll()
            #NewScript.FileName = self.FileName
            ...
            self.Outer['state'] = DISABLED
    def CompileFromIni(self):
        NewScript = GscFile(self.FileName, 1)
        NewScript.ReinitAll()
        NewScript.FileName = self.FileName
        NewScript.CompileIniToGsc()
        try:
            #NewScript = GscFile(self.FileName, 1)
            #NewScript.ReinitAll()
            #NewScript.FileName = self.FileName
            ...
            self.Outer.delete(1.0, END)
            if (self.Language == 'CHS'):
                self.Outer.insert(1.0, "Что-то пошло не так...\nНе удалось компилировать сей .ini...")
            else:
                self.Outer.insert(1.0, "Something went wrong...\nCouldn't compile this .ini...")
            self.Outer['state'] = DISABLED
CurrentSession = GUI()
